...
def process_row(index):
    ...
    protein_variants = []
    other_proteins = []
    gene_IDs = list(dict.fromkeys([ all_proteins[acc.split('.')[0]]['description'].split('gene:')[1].split('.')[0] for acc in protein_accessions if ('gene:' in all_proteins[acc.split('.')[0]]['description']) ]))
    SNPs = []
    all_ref_alleles = []
